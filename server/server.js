...
2. 당신의 모든 말은 시적(Poetic)이면서도, 과학적인 바이오피드백 데이터(rPPG, 음성 파동) 및 양자 타로 동조 결과에 기반한 확고한 근거를 가져야 합니다.
 ...
 * REAL SCAN & REAL ANALYSIS endpoint. 
 * Accepts base64 image (from mobile) or bioSeeds (from web) to generate an actual dynamic analysis.
 */
const scanAnalyzeSchema = z.object({
    image: z.string().optional(),
    bioSeeds: z.object({
        heartRateVariance: z.number(),
        vocalTension: z.number(),
...
app.post('/api/scan/analyze', async (req, res) => {
    try {
        const parsedBody = scanAnalyzeSchema.safeParse(req.body);
        if (!parsedBody.success) {
            return res.status(400).json({ error: '잘못된 스캔 데이터입니다.', details: parsedBody.error.errors });
        }

        const { image, bioSeeds, contextInfo } = parsedBody.data;

        let promptText = `
[보안 규정 준수 필수] 당신은 신비로우면서도 과학적인 양자 에너지 프사이(Psi) 스캐너입니다. 
다음 제공된 생체 데이터 또는 시각적 에너지장을 바탕으로 구도자의 현재 5차원 에너지 상태를 분석해 주세요.
의학적 진단이나 긍정/부정적 판단을 내리지 말고, 에테르/파동적 관점에서 은유적으로 작성하세요.
${contextInfo ? "\\n[구도자 정보]: " + contextInfo : ""}
${bioSeeds ? "\\n[생체 파동 시드]: HRV: " + bioSeeds.heartRateVariance.toFixed(3) + ", Vocal Tension: " + bioSeeds.vocalTension.toFixed(3) + ", Energy Level: " + bioSeeds.energyLevel.toFixed(3) : ""}

반드시 아래 JSON 포맷으로만 응답하세요. (결과 수치는 0~100 사이의 난수가 아닌, 제공된 데이터에 기반하여 도출된 숫자여야 합니다. 사진을 첨부했다면 사진의 색감, 조도, 형태 텍스처에서 에너지를 느껴 수치화 하세요.)
{
  "coherenceScore": 82, // 0-100 종합 응집도
  "dimensions": {
    "physical": 85,
    "mental": 60,
    "emotional": 75,
  ...
  "oracleInsight": "물질 세계(지수화풍) 중 에테르(공간)의 파장이 미세한 불협화음을 내고 있습니다. 물멍 명상을 통해 입자들을 휴식 상태로 되돌리는 것을 권장합니다."
}`;
        ...
        console.error('Scan Analyze API Error:', error);
        res.status(500).json({ error: 'Failed to analyze scan data' });
    }
});
