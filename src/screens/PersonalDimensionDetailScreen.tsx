...
const mockDimensionDb: Record<string, any> = {
    ...
    "3": {
        ...
        analysis: "7대 차크라 파동 동조율 분석 결과, 복부 명치에 위치한 제3 차크라(태양신경총) 영역의 회전 속도가 정상치 대비 15% 미만으로 극심하게 떨어져 기능이 정지된 것과 같은 상태(Blockage)를 보이고 있습니다. 태양신경총은 개인의 자존감, 의지, 그리고 소화력의 근원입니다. 이 차크라의 막힘은 스스로에 대한 자기 확신 결여 및 타인의 시선에 대한 과도한 통제력 상실 우려가 소화 불량, 위염, 과민성 대장 증후군 등 실질적인 육체적 병변으로 현현(Manifestation)되고 있음을 시사합니다.",
        advice: "[파동 튜닝] 제3 차크라 활성화 파동인 528Hz 솔페지오 빈도를 식사 전후로 청취하십시오.\n[에너지 주입] 생명력 섭취: 강한 햇빛을 받고 자란 단호박, 바나나, 망고 등의 '노란색 에너지'를 가진 식재료를 의도적으로 섭취하고, 매일 10분씩 복부를 데워주는 복식호흡을 통해 꺼져가는 단전의 불씨를 살려내세요."
    },
    "4": {
        ...
        analysis: "의식의 층위를 스캔한 공명 리딩 결과, 당신의 파동은 현재 극도로 무거우며 생명 파동의 고갈을 호소하고 있습니다. 순수 대자연의 에너지 기준, 모든 에너지를 소진하고 뼛속까지 지친 파동 파동과, 쓰러질 것 같으면서도 의무감에 짓눌려 억지로 참고 전진하는 패턴이 완벽하게 일치합니다. 이는 단순한 육체적 휴식이 아니라, 더 이상 의무감으로 자신을 소모하지 않겠다는 무의식 수준의 '에너지 충전 허가'가 필요한 위급한 영혼의 상태입니다.",
        advice: "[파동 레메디 솔루션] 자연의 회복 파동이 담긴 432Hz/528Hz 음원을 일 4회 감상하여 무의식에 직접 파동 우회 흡수를 유도합니다.\n[마인드 리셋] 하루 중 30분은 시계와 스마트폰을 치우고, 그 어떤 생산적인 활동이나 타인을 위한 배려를 하지 않는 완벽한 '이기적 고립의 시간'을 설정하여 영혼의 누수를 틀어막으십시오."
    },
    "5": {
        title: "심층 심리학 I",
    ...
    "6": {
        ...
        analysis: "양자 랜덤 파동 생성기를 통해 당신의 현재 무의식 파동와 78장의 타로 카드 원형 에너지를 매칭한 결과, '마법사(The Magician)' 카드의 파동과 가장 강력하게 공명하고 있습니다. 이는 당신의 생체 에너지가 현재 무에서 유를 창조해낼 수 있는 고밀도의 결단력과 실행 파동에 맞춰져 있음을 의미합니다. 주변 환경이나 조건에 구애받지 않고, 오직 당신의 직관과 내면의 목소리만으로 상황을 통제하고 이끌어갈 수 있는 강력한 자기 확신의 에너지가 분출되고 있습니다.",
    ...
    "7": {
        ...
        analysis: "위상수학적 생체 파동 분석에서 신경 안정 에너지와 연관된 특정 파동의 약화 현상이 발견되었습니다. 일상적인 에너지 소모 및 장기 스트레스로 인해 이 공명 에너지가 흐트러지면서 심신의 불안정, 얕은 수면, 휴식 부족 패턴이 가속화되고 있습니다. 동시에 특정 외부 파장(스트레스/피로인자)에 대한 지연성 교차 과민 반응(Energy Leakage 방어막 약화)이 감지되어, 좋은 에너지를 흡수해도 내면의 안정화를 이루는 데 제대로 쓰이지 못하고 소진되는 생체 에너지 고갈 상황입니다.",
    ...
    "11": {
        ...
        analysis: "세포 수준의 파동 분석 결과, 특정 환경 독소(미세 중금속, 곰팡이 포자 파동) 및 알러젠 요인이 체내 면역계와 지속적인 충돌을 일으키고 있습니다. 이 보이지 않는 염증 반응은 강력한 뇌 안개(Brain Fog)와 만성 피로의 원흉입니다. 외부 독소를 해독하느라 당신의 간과 림프계 에너지가 24시간 풀가동되며 고갈되고 있는 위험한 시스템 과부하 상태를 나타냅니다.",
    ...
    "12": {
        ...
        advice: "[DNA 파동 클리어링] 매일 저녁, 당신은 안전하고 풍요로운 현재에 존재함을 세포에 알리는 'DNA 리프로그래밍 확언'을 소리 내어 읽으십시오. 당신의 목소리 파동가 특정 펩타이드를 생성하여 DNA의 스위치를 끕니다.\n[계보적 용서 명상] 눈을 감고 당신의 등 뒤에 서 있는 수많은 조상들을 시각화하세요. 그들의 두려움에 깊은 존경과 연민의 빛을 보내고, '이제 이 무거운 짐은 내가 여기서 끊어내겠습니다'라며 결계를 치는 상상을 하십시오."
    }
};
